...
bool RgbImage::LoadBmpFilealpha( const char* filename ) 
{  
	Reset();
	char alpha;
	FILE* infile = fopen( filename, "rb" );		// Open for reading binary data
	...
	// Allocate memory
	int getal=GetNumBytesPerRowalpha();
	ImagePtr = new unsigned char[NumRows*GetNumBytesPerRowalpha()];
	if ( !ImagePtr ) {
		fprintf(stderr, "Unable to allocate memory for %ld x %ld bitmap: %s.\n", 
				NumRows, NumCols, filename);
		Reset();
	...
	for ( int i=0; i<NumRows; i++ ) {
		...
		for ( j=0; j<NumCols; j++ ) {
			...
			*cPtr = fgetc( infile );		// Red color value
			alpha=255;
			if((*(cPtr+2)==255 && *(cPtr+1)==255 && *(cPtr+0)==255)|| j%5==0) alpha=0;
			*(cPtr+3) = alpha;	// alpha color value
			cPtr += 4;
